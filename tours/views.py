...
def main_view(request):
    id_tours = [*tours_all]
    random_id_tours = [id for id in random.sample(id_tours, 6)]
    tours = {'random_6_tours': [tours_all[id] for id in random_id_tours]}

    return render(request, 'index.html', context=tours)


def departure_view(request, departure: str):
    tours_filter = [tours_all[tour] for tour in tours_all if tours_all[tour]['departure'] == departure]

    additional_dict = {}
    additional_dict['from'] = departures[departure]
    additional_dict['count'] = len(tours_filter)
    additional_dict['price_min'] = min([tour['price'] for tour in tours_filter])
    additional_dict['price_max'] = max([tour['price'] for tour in tours_filter])
    additional_dict['night_min'] = min([tour['nights'] for tour in tours_filter])
    additional_dict['night_max'] = max([tour['nights'] for tour in tours_filter])
    additional_dict[departure] = 'active'

    tours = {'tours': tours_filter} | additional_dict
...
def tour_view(request, id: int):
    ...
    stars = "★" * int(tours_all[id]['stars'])
    return render(request, 'tour.html', context={**tours_all[id], 'from': departure_rus, 'stars_conc': stars,
                                                 departure: 'active'})
